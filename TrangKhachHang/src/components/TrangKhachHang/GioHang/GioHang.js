...
export default class GioHang extends Component {
        ...
        if (this.state.products.length > 0) {
            ...
            return (
                this.state.products.map(product => {
                    i = i + 1;
                    return <ItemGioHang data={{ _id: product.productId._id, index: i, img: product.productId.imageUrl, price: product.productId.price, title: product.productId.title, quantity: product.quantity }} token={this.props.token} onDelete={this.delete} updateQuantity={this.updateQuantity}></ItemGioHang>
                })
            );
        }
    };
    ...
    showButton = () => {
        if (this.state.products.length > 0){ 
            let totalPrice = this.state.products.map( item => item.productId.price * item.quantity).reduce((accumulator, currentValue) => accumulator + currentValue);
            return (
                <div className="row mt-3 mb-3">
                    <div className="col-9 ">
                        <strong style={{float: "right"}}>Tổng tiền: {totalPrice}</strong>
                    </div>
                    <div className="col-3">
                        {/* <a href="#" class="genric-btn success radius" onClick={(e) => this.createOrder(e)} >Đặt hàng</a> */}
                        <a href="#" class="genric-btn success radius" style={{float: "right"}} onClick={(e)=> this.submit(e)} >Đặt hàng</a>
                    </div>
